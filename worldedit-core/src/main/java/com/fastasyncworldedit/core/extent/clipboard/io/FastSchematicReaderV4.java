...
public class FastSchematicReaderV4 implements ClipboardReader {
    ...
    public FastSchematicReaderV4(@NonNull InputStream stream) {
    ...
    private void readBiomes(Clipboard target) throws IOException {
        ...
        boolean hasClipboard = target != null;
        byte paletteType = CACHE_IDENTIFIER_BIOMES;
        BooleanSupplier paletteAlreadyInitialized = () -> this.biomePalette[0] != null;
        PaletteInitializer paletteInitializer = this.provideBiomePaletteInitializer();
        PaletteDataApplier paletteDataApplier = this.getBiomeWriter(target);
        AdditionalTagConsumer additionalTag = (type, tag) -> {
            try {
                this.nbtInputStream.readTagPayloadLazy(type, 0);
            } catch (IOException e) {
        ...
        if (this.dataCacheWriter == null) {
            ...
            this.dataCacheWriter = new FastBufferedOutputStream(new LZ4BlockOutputStream(byteArrayOutputStream)) {
                ...
                public void close() throws IOException {
                    ...
                    super.close();
                    FastSchematicReaderV4.this.dataCache = byteArrayOutputStream.toByteArray();
                }
            };
        }
        ...
        if (this.paletteCacheWriter == null) {
            ...
            this.paletteCacheWriter = new FastBufferedOutputStream(new LZ4BlockOutputStream(byteArrayOutputStream)) {
                ...
                public void close() throws IOException {
                    this.write(CACHE_IDENTIFIER_END);
                    super.close();
                    FastSchematicReaderV4.this.paletteCache = byteArrayOutputStream.toByteArray();
                }
            };
        }
