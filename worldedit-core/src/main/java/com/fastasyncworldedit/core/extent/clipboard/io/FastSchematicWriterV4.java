...
public class FastSchematicWriterV4 implements ClipboardWriter {
    ...
    public FastSchematicWriterV4(final NBTOutputStream outputStream) {
    ...
    private void writeBiomes(NBTOutputStream biomes, Clipboard clipboard) throws IOException {
        NBTOutputStream out = biomes;
        int capacity = BiomeType.REGISTRY.size();
        Function<BlockVector3, BiomeType> objectResolver = pos -> pos.getBiome(clipboard);
        Function<BiomeType, Character> ordinalResolver = biome -> (char) biome.getInternalId();
        Function<BiomeType, String> paletteEntryResolver = BiomeType::id;
        int dataBytesUsed = 0;
        ...
        try (LZ4BlockOutputStream dataOut = new LZ4BlockOutputStream(bytes)) {
            ...
            while (iterator.hasNext()) {
                ...
                BiomeType obj = objectResolver.apply(pos);
                    ...
                    out.writeLazyCompoundTag(
                            "Platforms", platforms -> {
                                for (final Platform platform : WorldEdit.getInstance().getPlatformManager().getPlatforms()) {
                                    platforms.writeLazyCompoundTag(
                                            platform.getId(), p -> {
